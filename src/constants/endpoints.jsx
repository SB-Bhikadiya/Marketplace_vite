export const BASE_URL = "http://192.168.29.167:3000";
export const REGISTER_ENDPOINT = "/users";
export const LOGIN_ENDPOINT = "/users/login";

export const USER_ENDPOINT = "/users";
export const COLLECTION_ENDPOINT = "/collection";
export const ACCEPTED_ENDPOINT = "/accepted";
export const BOUGHT_ENDPOINT = "/bought";
export const CANCELED_OFFERED_ENDPOINT = "/canceled-offered";
export const CREATED_AUCTION_ENDPOINT = "/created-auction";
export const LISTED_ENDPOINT = "/listed";
export const OFFERED_ENDPOINT = "/offered";
export const PLACED_BID_ENDPOINT = "/placed-bid";
export const RESULTED_AUCTION_ENDPOINT = "/resulted-auction";
export const TOKEN_ENDPOINT = "/token";
export const METADATA_ENDPOINT = "/metadata";
export const NFTS_ENDPOINT = "/nfts";
